...
class NIPS_TS_GECCOSegLoader(object):
    def __init__(self, data_path, win_size, step, mode="train"):
        ...
        self.win_size = win_size
        self.scaler = StandardScaler()
        data = np.load(data_path + "/NIPS_TS_GECCO_train.npy")
        self.scaler.fit(data)
        data = self.scaler.transform(data)
        test_data = np.load(data_path + "/NIPS_TS_GECCO_test.npy")
        self.test = self.scaler.transform(test_data)

        self.train = data
        self.val = self.test
        self.test_labels = np.load(data_path + "/NIPS_TS_GECCO_test_label.npy")
        print("test:", self.test.shape)
        print("train:", self.train.shape)
...
def get_loader_segment(data_path, batch_size, win_size=100, step=100, mode='train', dataset='KDD'):
    if (dataset == 'SMD'):
        ...
        dataset = ServiceSegLoader(data_path, win_size, 1, mode)
    elif (dataset == 'MSL'):
        ...
        dataset = NIPS_TS_GECCOSegLoader(data_path, win_size, 1, mode)
